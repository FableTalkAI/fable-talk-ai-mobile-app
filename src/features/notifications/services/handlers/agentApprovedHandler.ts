...
export const agentApprovedHandler = async ({ agentId }: AgentModerationPayload) => {
  const myAgents = myAgentsSelector(store.getState());
  const isNeedsUpdate = !myAgents.some(a => a.id === agentId && a.moderationStatus === AgentModerationStatus.Approved);

  if (isNeedsUpdate) {
    await Promise.all([store.dispatch(getMyAgents(false)).unwrap(), store.dispatch(getFilteredAgents(false)).unwrap()]);
  }

  navigate('TabBarNavigator', { screen: 'Home', params: { tabIndex: 1 } });
};
